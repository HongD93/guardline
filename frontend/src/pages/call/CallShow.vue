...
const { status, finalTurns, partialText, errorMessage, startMic, playScenario, stop } = useCallStream();
...
const loadScenarios = async () => {
  try {
    ...
    scenarios.value = spec.scenarios.map(({ id, title, kind, expected }) => ({ id, title, kind, expected }));
...
<template>
  <main class="page">
    <header class="header">
      ...
      <p class="subtitle">실시간 전사 파이프라인 (1주차)</p>
    </header>
    ...
    <p v-if="selected" class="expected">
      기대 판정 · <strong>{{ selected.expected.level }}</strong> ({{ selected.expected.score }}점)
      — 판정 엔진은 2주차에 연결합니다.
    </p>
